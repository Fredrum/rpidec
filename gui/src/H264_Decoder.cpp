  ...
  ,frame_delay(0)
{
  ...
  pi_decoder = (ChiakiPiDecoder*)malloc(sizeof(ChiakiPiDecoder));
  ChiakiLog log;
  chiaki_pi_decoder_init(pi_decoder, &log);
  chiaki_pi_decoder_set_params(pi_decoder, 0, 0, 640, 480, 1);
}
...
bool H264_Decoder::load(std::string filepath, float fps) {
  ...
  if(fps > 0.0001f) {
    ...
    frame_timeout = 1.0f + frame_delay;
  }
...
bool H264_Decoder::update(bool& needsMoreBytes) {
  ...
  if(len) {
    //decodeFrame(&buffer[0], size);//orig
    push_buffer(pi_decoder, &buffer[0], size); //rpi
    //printf("In place of frame decode:  %d\n", size); // matches the pkt sized I logged to file!
